...
const QUICK_ACTIONS: QuickAction[] = [
  {
    ...
    hint: "Новое событие для участников",
    icon: "Plus",
    view: "add",
  ...
  },
  {
    ...
    icon: "ClipboardList",
    view: "event-signups",
    accent: "bg-white border hover:bg-gray-50 text-gray-800",
  },
  {
    label: "Заявки на роли",
    hint: "Одобрить мастеров и организаторов",
    icon: "Shield",
    view: "roles",
    accent: "bg-white border hover:bg-gray-50 text-gray-800",
  },
  {
    label: "Статьи блога",
    hint: "Промодерировать публикации",
    icon: "BookOpen",
    view: "blog",
    accent: "bg-white border hover:bg-gray-50 text-gray-800",
  },
    ...
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-gray-500 mt-1 text-sm">
          Здесь вы управляете всем контентом и пользователями сайта
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {QUICK_ACTIONS.map((action) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${action.view === "add" ? "bg-white/20" : "bg-gray-100"}`}
                >
                  <Icon
                    name={action.icon}
                    size={16}
                    className={
                      action.view === "add" ? "text-white" : "text-gray-600"
                    }
                  />
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div
                    className={`text-xs truncate ${action.view === "add" ? "text-white/70" : "text-gray-400"}`}
                  >
                    {action.hint}
                  </div>
                </div>
                ...
                upcoming.map((event, idx) => (
                  ...
                  >
                    ...
                    {event.total_spots ? (
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${OCCUPANCY_COLOR[event.occupancy || "low"]}`}
                      >
            ...
            <CardContent className="p-0">
              {recent.length === 0 ? (
                ...
                recent.map((event, idx) => (
                  ...
                  >
                    <div
                      className={`w-2 h-2 rounded-full flex-shrink-0 mt-0.5 ${event.is_visible ? "bg-green-400" : "bg-gray-300"}`}
                    />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-800 truncate">
                        {event.title}

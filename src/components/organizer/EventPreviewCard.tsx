    ...
    <Card className="bg-card border shadow-md overflow-hidden max-w-sm">
      {data.image_url ? (
        <div className="relative h-44 overflow-hidden">
          <img src={data.image_url} alt={data.title || "Мероприятие"} className="w-full h-full object-cover" />
      ...
      <CardContent className="p-5">
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          <Button size="sm" className="rounded-full text-xs h-8 px-4">Подробнее</Button>
        </div>
      </CardContent>
    </Card>
  );
      ...
      {data.image_url ? (
        <div className="relative h-40 overflow-hidden">
          ...
          <div className="absolute bottom-3 left-3 right-3">
            <span className={`inline-block text-xs px-2.5 py-1 rounded-full font-medium ${typeColors.bg} ${typeColors.color} mb-1.5`}>
              {typeLabel}
            </span>
            <h2 className="text-white font-bold text-base leading-tight">
              {data.title || <span className="italic opacity-70">Название мероприятия</span>}
            </h2>
          </div>
        </div>
      ) : (
        <div className="p-4 bg-muted/40 border-b">
          ...
          <h2 className="font-bold text-base leading-tight">
            {data.title || <span className="italic text-muted-foreground">Название мероприятия</span>}
          </h2>
        </div>
      )}
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          {dateObj && (
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="font-medium text-xs">{format(dateObj, "d MMMM yyyy, EEEE", { locale: ru })}</div>
                {data.start_time && (
                  <div className="text-muted-foreground text-xs">{data.start_time}{data.end_time ? ` — ${data.end_time}` : ""}</div>
                )}
              </div>
            </div>
          )}
          {data.bath_name && (
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                {data.bath_address && <div className="text-muted-foreground text-xs">{data.bath_address}</div>}
              </div>
            </div>
          )}
          {totalSpots > 0 && (
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                {spots && <div className={`text-xs font-medium ${spots.color}`}>{spots.text}</div>}
              </div>
            </div>
          )}
        </div>
        ...
        {data.full_description && (
          <div>
            ...
            <p className="text-muted-foreground text-xs leading-relaxed whitespace-pre-line">{data.full_description}</p>
          </div>
        )}
          ...
          <div>
            ...
            <div className="space-y-2">
              {data.rules.filter(Boolean).map((rule, i) => (
                <div key={i} className="flex items-start gap-2">
                  <Icon name="Shield" size={13} className="text-accent mt-0.5 flex-shrink-0" />
        ...
        <Card className="border shadow-sm">
          <CardContent className="p-4">
            {data.pricing_type === 'dynamic' && data.pricing_tiers?.length ? (
              ...
              <>
                {priceDisplay && !data.pricing_lines?.length && (
                  <div className="text-2xl font-bold text-accent mb-2">{priceDisplay}</div>
                )}
                {data.pricing_lines && data.pricing_lines.filter(Boolean).length > 0 && (
                  <div className="mb-3">
                    <div className="text-xs font-semibold text-foreground mb-1.5">СТОИМОСТЬ УЧАСТИЯ</div>
            ...
            {totalSpots > 0 && (
              <div className={`inline-block text-xs px-2.5 py-1 rounded-full font-medium ${spotsColor} mb-3`}>
                {spotsLeft === 0
                  ? "Мест нет"
                  : spotsLeft <= 2
            ...
            <Button size="sm" className="w-full mt-3 text-xs">Записаться</Button>
          </CardContent>
        </Card>
      </div>
    </div>
...
export default function EventPreviewCard({ data }: EventPreviewCardProps) {
  ...
  return (
    <div className="sticky top-20">
      <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-3">Предпросмотр</p>
      ...
      {tab === "card" ? (
        <>
          ...
          <p className="text-xs text-muted-foreground mt-3 text-center">Так карточка будет выглядеть в каталоге</p>
        </>
      ) : (
        <>
          <FullPreview data={data} />
          <p className="text-xs text-muted-foreground mt-3 text-center">Так будет выглядеть страница встречи</p>
        </>
      )}
    </div>
  );

...
interface BadgesResult {
  badges: AdminBadges;
  highTickets: AdminInsights["highTickets"];
  recentBookings: AdminInsights["recentBookings"];
  pendingModerations: AdminInsights["pendingModerations"];
}

async function fetchAdminBadges(): Promise<BadgesResult> {
  ...
  const recentBookings = calcBookings.filter((b) => b.status === "new").slice(0, 5);
  ...
  return { badges, highTickets, recentBookings, pendingModerations };
}

export const ADMIN_BADGES_QUERY_KEY = ["admin-badges"] as const;

/**
 * Агрегирует счётчики и инсайты для админ-панели.
 * Кэшируется на 30 секунд, в фоне обновляется каждую минуту.
 * Внутри одной сессии переключение между вкладками админки не тянет 8 запросов заново.
 */
export function useAdminBadges() {
  ...
  const badges = data?.badges ?? EMPTY_BADGES;
  const insights: AdminInsights = {
    badges,
    loading: isLoading,
    events: [],
    highTickets: data?.highTickets ?? [],
    recentBookings: data?.recentBookings ?? [],
    pendingModerations: data?.pendingModerations ?? [],
  };

  const refresh = () => queryClient.invalidateQueries({ queryKey: ADMIN_BADGES_QUERY_KEY });

...
const Events = () => {
  const upcomingEvents = [
    {
      id: 1,
      title: 'Тёплые Знакомства: Осенний круг',
      date: '25 сентября 2025',
      time: '19:00 — 22:00',
      type: 'Знакомства',
      location: 'Баня на Чистых Прудах',
      price: '5 000 ₽',
      description: 'Камерная встреча для 8 человек. Через ритуалы парения и чайные церемонии создаём пространство искренности.',
      spotsLeft: 3,
      totalSpots: 8,
      icon: 'Users'
    },
    {
      ...
      price: 'от 12 000 ₽/чел',
      description: 'Полный день трансформации для команды до 15 человек. Снимаем напряжение и строим доверие.',
      spotsLeft: 15,
      totalSpots: 15,
      icon: 'HandHeart'
    },
    {
      ...
      price: '3 500 ₽',
      description: 'Изучаем древние практики работы с паром, дыханием и энергией пространства.',
      spotsLeft: 2,
      totalSpots: 12,
      icon: 'Flame'
    },
    {
      ...
      description: 'Особый формат для трёх пар. Углубляем близость через совместные практики.',
      spotsLeft: 1,
      totalSpots: 3,
      icon: 'Heart'
    }
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'Знакомства': return 'bg-blue-100 text-blue-800';
      case 'Тимбилдинг': return 'bg-green-100 text-green-800';
      case 'Обучение': return 'bg-purple-100 text-purple-800';
      case 'Свидания': return 'bg-pink-100 text-pink-800';
      ...
      <div className="container mx-auto px-6 py-12">
        ...
        <section className="text-center mb-16">
          ...
          <p className="text-xl text-nature-forest/80 max-w-3xl mx-auto leading-relaxed">
            Присоединяйтесь к нашим мероприятиям, где каждая встреча — это возможность 
            для глубокой трансформации и искренних связей.
          </p>
        </section>
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          {upcomingEvents.map((event) => (
            <Card key={event.id} className="bg-nature-cream/95 border-nature-brown/20 overflow-hidden hover:shadow-lg transition-all duration-300">
                  ...
                  <div className="lg:col-span-2 p-8">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <Icon name={event.icon as any} size={24} className="text-nature-brown" />
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${getTypeColor(event.type)}`}>
                          {event.type}
                        </span>
                      </div>
                      <div className="text-right">
                        <div className="text-sm text-nature-forest/60">{event.date}</div>
                        <div className="text-sm font-medium text-nature-forest">{event.time}</div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-4 text-sm text-nature-forest/70 mb-6">
                      <div className="flex items-center gap-2">
                        <Icon name="MapPin" size={16} className="text-nature-brown" />
                        {event.location}
                      </div>
                      <div className="flex items-center gap-2">
                        <Icon name="Users" size={16} className="text-nature-brown" />
                    ...
                    <div className="mb-4">
                      <div className="flex justify-between text-xs text-nature-forest/60 mb-1">
                        ...
                        <span>{Math.round(((event.totalSpots - event.spotsLeft) / event.totalSpots) * 100)}%</span>
                      </div>
        ...
        <Card className="max-w-2xl mx-auto mt-16 bg-nature-brown/5 border-nature-brown/30">
          <CardContent className="p-8 text-center">
            <Icon name="Bell" size={32} className="text-nature-brown mx-auto mb-4" />

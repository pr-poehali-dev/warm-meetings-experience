...
const BENEFITS = [
  { icon: "Users", title: "Готовая аудитория", desc: "Более 500 участников уже ищут новые встречи. Твоя афиша попадёт в ленту активных и заинтересованных людей." },
  ...
  { icon: "CreditCard", title: "Приём оплат онлайн", desc: "Безопасные платежи, автоматические чеки. Деньги приходят после проведения встречи." },
  { icon: "Bell", title: "Автоматические уведомления", desc: "Участники получают напоминания о встрече. Меньше неявок, больше стабильности." },
  { icon: "BarChart3", title: "Статистика и аналитика", desc: "Видишь, сколько людей смотрят встречу, записываются и возвращаются снова." },
...
const STEPS = [
  ...
  { num: "02", title: "Создание", desc: "Поможем создать карточку встречи и настроить всё под твой формат" },
  { num: "03", title: "Публикация", desc: "Встреча появляется в афише и становится доступна для записи" },
  { num: "04", title: "Проведение", desc: "Ты проводишь встречу, мы собираем обратную связь и переводим оплату" },
];
...
const TARIFFS = [
  ...
  { name: "Партнёрский", commission: "10%", badge: "3+ встреч / мес", features: ["Размещение в ленте", "Профиль организатора", "Помощь в продвижении", "Приоритетная поддержка"], highlighted: true },
];

const FAQ = [
  { q: "Сколько стоит размещение?", a: "Размещение встреч бесплатно. Мы берём комиссию 10–15% с каждого проданного билета — только если встреча состоялась и собрала участников." },
  ...
  { q: "Как быстро встреча появится в афише?", a: "После модерации — в течение 24 часов. В высокий сезон может быть чуть дольше, но мы предупреждаем." },
  { q: "Что если встреча не наберёт участников?", a: "Ты сам решаешь минимальный порог. Если он не набран — встреча отменяется без последствий. Никаких штрафов." },
  { q: "Можно ли проводить встречи в своей бане?", a: "Да. Если у тебя есть своя баня — отлично. Если нет — поможем подобрать площадку из наших партнёров." },
  { q: "Какие форматы встреч подходят?", a: "Банные встречи, мастер-классы по парению, чайные церемонии, восстановительные практики — любой формат, связанный с баней и здоровьем." },
];

const CHECKLIST = [
  "Идея встречи (формат, тема, программа)",
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative min-h-[70vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center py-32">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">Проводи встречи вместе со{"\u00A0"}СПАРКОМом</h1>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-3 gap-8 max-w-3xl mx-auto text-center">
            <div>
              ...
              <div className="text-sm text-muted-foreground">Проведено встреч</div>
            </div>
            <div>
